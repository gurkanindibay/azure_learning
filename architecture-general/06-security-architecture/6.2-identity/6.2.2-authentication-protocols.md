...
- [5. Authorization vs. Identity Protocols](#5-authorization-vs-identity-protocols) — OAuth2, OAuth2+PKCE, OIDC, OIDC+PKCE
- [6. Single Sign-On (SSO) & Identity Protocols](#6-single-sign-on-sso--identity-protocols) — SAML, OIDC SSO
...
Legacy (Basic/Digest) → API Keys → Session-Based → Token-Based (JWT)
    → Authorization Protocols (OAuth2, PKCE) → Identity Protocols (OIDC) → SSO
```
