...
PKCE is the default hardening mechanism for OAuth 2.0 authorization code flow, especially for public clients (SPA, mobile, desktop). It binds the authorization code to the client instance that initiated the flow, reducing interception risk.

| Component | Purpose |
|-----------|---------|
| **code_verifier** | One-time, high-entropy secret generated by the client |
| **code_challenge** | Hashed form of the verifier sent in authorization request |
| **Token exchange check** | Authorization server validates verifier against stored challenge |

Use these decision rules at architecture level:

- **Required**: SPA, mobile, desktop clients.
- **Recommended**: Server-side web apps (defense in depth; OAuth 2.1 direction).
- **Not applicable**: Client-credentials flow (machine-to-machine).
