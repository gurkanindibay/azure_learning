...
description: "This document is part of a three-level authentication and identity reference. Read them in order for a complete top-to-bottom understanding:"
...
This document is part of a three-level authentication and identity reference. Read them in order for a complete top-to-bottom understanding:

```
┌─────────────────────────────────────────────────────────────────────────┐
│                AUTHENTICATION & IDENTITY — READING PATH                 │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  Level 1 — Method Catalog (START HERE)                                  │
│  ┌─────────────────────────────────────────────────────────────┐        │
│  │ Authentication Methods Overview              ← YOU ARE HERE │        │
│  │ What methods exist? When to use each?                       │        │
│  │ Decision matrices, security comparison, viable scenarios    │        │
│  └──────────────────────────┬──────────────────────────────────┘        │
│                             │                                           │
│                             ▼                                           │
│  Level 2 — Protocol Deep Dives                                          │
│  ┌─────────────────────────────────────────────────────────────┐        │
│  │ Authentication Methods — Detailed Includes                  │        │
│  │ How do the protocols work step by step?                     │        │
│  │ Sequence diagrams, implementation code, token handling      │        │
│  └──────────────────────────┬──────────────────────────────────┘        │
│                             │                                           │
│                             ▼                                           │
│  Level 3 — Architecture & Governance                                    │
│  ┌─────────────────────────────────────────────────────────────┐        │
│  │ 6.2 Identity Architecture                                   │        │
│  │ Identity operating model, IdP selection, federation,        │        │
│  │ lifecycle management, governance, authorization models      │        │
│  └─────────────────────────────────────────────────────────────┘        │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
|-------|----------|-------|
| **1** | **6.2.1 Authentication Methods** ← You are here | Method catalog, selection criteria, decision matrices |
...
- **Token & Protocol-Based Methods** → [Protocol Deep Dives](./6.2.2-authentication-protocols.md)
...
- **Certificate & Network Authentication**
  ...
  - [9. NTLM](#9-ntlm)
- **Biometric & Key-Based Methods**
  - [10. Biometric Authentication](#10-biometric-authentication)
  - [11. SSH Keys](#11-ssh-keys)
- **Cloud-Native Identity (Azure)** → [Identity Architecture](./index.md)
...
Authentication is the process of verifying the identity of a user, system, or service. Choosing the right authentication method depends on security requirements, user experience, and the type of system being secured.

This document is the **base authentication reference** — start here to understand what methods exist, when each is appropriate, and how they compare. Methods are organized from simple credential-based approaches through protocol-based standards to cloud-native identity patterns.
...
> ### Credential-Based Methods
> Methods 1–3 authenticate using credentials the user or system provides directly (passwords, second factors, API keys).
...
> ### Token & Protocol-Based Methods
> Methods 4–6 use standardized protocols and token formats. This section covers **what** each protocol does and **when** to use it.
>
> **→ Protocol Deep Dives**: For step-by-step sequence diagrams, implementation code (React, .NET), and token handling patterns, see [6.2.2 Authentication Protocols](./6.2.2-authentication-protocols.md).
...
> **→ Protocol Deep Dives**: [OAuth2 Authorization Code flow](./6.2.2-authentication-protocols.md#oauth2-authorization-framework) • [OAuth2 + PKCE](./6.2.2-authentication-protocols.md#oauth-20--pkce-public-client-mode) • [OpenID Connect](./6.2.2-authentication-protocols.md#openid-connect-oidc) • [OIDC + PKCE](./6.2.2-authentication-protocols.md#openid-connect--pkce-recommended-for-spamobile)
...
> **→ Protocol Deep Dives**: [JWT Bearer flow with code examples](./6.2.2-authentication-protocols.md#bearer-authentication--jwt) • [Access & Refresh Token patterns](./6.2.2-authentication-protocols.md#access--refresh-tokens)
...
> ### Certificate & Network Authentication
> Methods 7–9 authenticate using certificates, tickets, or challenge-response mechanisms at the network/transport level.
...
> ### Biometric & Key-Based Methods
> Methods 10–11 authenticate using biological characteristics or asymmetric key pairs.
...
> ### Cloud-Native Identity (Azure)
> Methods 12–14 are Azure-specific identity patterns that eliminate credential management for cloud workloads.
>
> **→ Architecture Context**: For identity operating model, IdP selection, and managed identity architecture patterns, see [6.2 Identity Architecture](./README.md#managed-identity-architecture).
...
> ### Fine-Grained Access Tokens
> Methods 15–16 provide scoped, time-limited access to specific Azure resources following the principle of least privilege.
