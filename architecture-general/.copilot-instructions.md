...
└── 10-practicality-taxonomy/ → Taxonomy source + cross-cutting
```
...
The `architecture_taxonomy_reference.md` file is **automatically generated** from README.md files:

```bash
# Regenerate taxonomy reference
python scripts/sync_taxonomy_reference.py

# Check if sync is needed (useful for CI)
...
4. After editing README.md, run `python scripts/sync_taxonomy_reference.py`
...
Use Archify as the primary tool for substantial architecture diagrams. Keep the Archify JSON specification, delivered HTML viewer, and PNG fallback in the document's local `resources/` directory. Run Archify `validate`, `deliver`, and `visual-check` before handoff. Embed the PNG with standard Markdown and link the HTML viewer separately because iframe rendering is not reliable across Markdown viewers. Use Mermaid with consistent styling for small inline explanatory diagrams:
