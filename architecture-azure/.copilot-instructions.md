...
Use Archify as the primary tool for substantial Azure architecture diagrams. Store the Archify JSON specification, delivered HTML viewer, and PNG fallback in the document's local `resources/` directory. Run Archify `validate`, `deliver`, and `visual-check` before handoff. Embed the PNG with standard Markdown and link the HTML viewer separately because iframe rendering is not reliable across Markdown viewers. Use Mermaid for small inline diagrams:
