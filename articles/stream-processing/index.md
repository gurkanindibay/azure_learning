---
type: Index
title: "Stream Processing — Source Articles"
description: "Source articles for stream processing system-design topics."
timestamp: 2026-06-27T00:00:00Z
---

# Stream Processing — Source Articles

> **Parent**: [Articles Overview](../index.md)
> **Related**: [System Design Architecture → stream-processing](../../system-design-architecture/stream-processing/)

Source articles covering stream processing patterns, strategies, and case studies.

## Articles

- [apache-flink-10000-feet/01-apache-flink-from-10000-feet](apache-flink-10000-feet/01-apache-flink-from-10000-feet.md)
- [async-patterns-java/01-senior-java-concurrency-patterns](async-patterns-java/01-senior-java-concurrency-patterns.md)
- [async-patterns-java/02-dotnet-async-concurrency-patterns](async-patterns-java/02-dotnet-async-concurrency-patterns.md)
- [Behind the Scenes: Building a Robust Ads Event Processing Pipeline (Part I)](netflix-ads-event-processing-pipeline.md)
- [Behind the Scenes: Evolving Netflix's Ads Event Pipeline for Live (Part II)](netflix-ads-event-pipeline-live.md)
