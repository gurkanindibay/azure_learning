---
type: Index
title: "Resilience — Source Articles"
description: "Source articles for resilience system-design topics."
timestamp: 2026-06-27T00:00:00Z
---

# Resilience — Source Articles

> **Parent**: [Articles Overview](../index.md)
> **Related**: [System Design Architecture → resilience](../../system-design-architecture/resilience/)

Source articles covering resilience patterns, strategies, and case studies.

## Articles

- [What 5 Famous Outages Taught Me About System Design](What 5 Famous Outages Taught Me About System Design.md)
- [defensive-coding-approach](defensive-coding-approach.md)
- [one-microservice-failure-takes-down-entire-platform](one-microservice-failure-takes-down-entire-platform.md)
- [otp-service-fails-during-peak-traffic](otp-service-fails-during-peak-traffic.md)
- [resilience-patterns-in-distributed-systems](resilience-patterns-in-distributed-systems.md)
- [uber-intelligent-load-management](uber-intelligent-load-management.md)
- [your-circuit-breaker-lying-to-you](your-circuit-breaker-lying-to-you.md)
