...
![Image represents a simple database table schema diagram for a leaderboard. The diagram is a rectangular box with a header and a body. The header, positioned at the top, boldly displays the table name 'leaderboard'.  The body, separated from the header by a horizontal line, contains two rows defining the table's columns. The first row specifies a column named 'user_id' with a data type of 'varchar,' indicating it will store variable-length strings representing user identifiers. The second row defines a column named 'score' with a data type of 'int,' signifying it will store integer values representing user scores.  No connections or information flow is depicted beyond the table's internal structure.](images/images-courses-system-design-interview-real-time-gaming-leaderboard-figure-5-leaderboard-table-WGJD5HTB.png)
...
[https://medium.com/@sandeep4.verma/building-real-time-leaderboard-with-redis-82c98aa47b9f](https://medium.com/%40sandeep4.verma/building-real-time-leaderboard-with-redis-82c98aa47b9f)
